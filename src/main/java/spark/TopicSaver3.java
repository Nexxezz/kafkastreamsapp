...
public class TopicSaver3 {
    ...
    public static void main(String[] args) {
        ...
        try (SparkSession ss = initSpark();
             KafkaConsumer<byte[], HotelWeather> consumer = getConsumer(getProps(groupId), topic)) {

            ArrayList<HotelWeather> buffer = new ArrayList<>(RECORDS_BUFFER_SIZE);
            int totalRead = 0;
            boolean isTopicEmpty = false;

            while (!isTopicEmpty) {
                List<HotelWeather> records = Collections.emptyList();
                final ConsumerRecords<byte[], HotelWeather> consumerRecords = consumer.poll(KAFKA_POLL_TIMEOUT_MS);
...
//                    }
    ...
    private static void saveToHdfs(List<HotelWeather> buffer, List<HotelWeather> records,
